...
public class IntegrationTests {
	...
	public void test() {
		...
		LinkedList<Ray> rayList = findRaysThroughVpPixles(camera, 3, 3);		
		...
		rayList = findRaysThroughVpPixles(camera, 3, 3);		
		...
		camera = new Camera(new Point3D(0, 0, 0.5), new Vector(0, 0, 1), new Vector(0, -1, 0)).setDistance(1);
		rayList = findRaysThroughVpPixles(camera, 3, 3);		
		plane = new Plane(new Point3D(0, 0, 4), new Vector(new Point3D(0, -2, 0)));
		assertEquals("Wrong number of intersections", 9, countIntersections(rayList, plane));
		
		// #13:
		camera = new Camera(new Point3D(0, 0, 0), new Vector(0, 0, 1), new Vector(0, -1, 0)).setDistance(1);
		rayList = findRaysThroughVpPixles(camera, 3, 3);		
		plane = new Plane(new Point3D(0 ,0, 6), new Vector(new Point3D(0, -4, 0)));
	...
	{
		int counter = 0;
		for (Ray ray : rayList)	// The loop checks intersections for each shape
		{			
